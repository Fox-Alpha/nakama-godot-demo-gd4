...
func add_character(plyname: String, color: Color) -> Node:
	var listing := CharacterListing.instantiate()
	add_child(listing)
	listing.setup(plyname, color)
	#warning-ignore: return_value_discarded
	listing.connect("requested_deletion", _on_CharacterListing_requested_deletion)
	#warning-ignore: return_value_discarded
	listing.connect("character_selected", _on_CharacterListing_character_selected)
	# warning-ignore:return_value_discarded
	listing.connect("character_accepted", _on_CharacterListing_character_accepted)
	return listing
